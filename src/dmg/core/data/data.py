...
def split_dataset_by_basin(
    ...
    try:
        ...
        subset_path = obs_cfg.get('subset_path') or obs_cfg.get('station_ids')
        if not subset_path:
            raise ValueError(
                "Neither 'subset_path' nor 'station_ids' is set in observations config"
            )
        ...
        for key, tensor in dataset.items():
            ...
            if not torch.is_tensor(tensor):
                # Non-tensor values (e.g. denorm_fn) are kept as-is
    ...
    for i, date_str in enumerate(date_range):
        try:
            ...
            if isinstance(date_str, str):
                ...
                if 'T' in date_str:
                    date_obj = dt.datetime.fromisoformat(
