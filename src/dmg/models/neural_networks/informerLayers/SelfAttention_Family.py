...
from dmg.models.neural_networks.informerLayers.masking import (
    ProbMask,
...
class ProbAttention(nn.Module):
    ...
    def _prob_QK(self, Q, K, sample_k, n_top):  # n_top: c*ln(L_q)
        ...
        # ---- safety clamps (belt-and-suspenders) ----
        sample_k = max(1, min(L_K, sample_k))
        n_top = max(1, min(L_Q, n_top))
        ...
        # [B, H, L_q, 1, E] x [B, H, L_q, E, sample_k] -> [B, H, L_q, 1, sample_k]
        ...
        M_top = M.topk(n_top, sorted=False)[1]  # indices in L_q dim

        # use the reduced Q to calculate full Q_K
        Q_reduce = Q[
    ...
    def _get_initial_context(self, V, L_Q):
        ...
        else:  # use mask, requires L_Q == L_V (self-attention)
            assert L_Q == L_V
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        # to [B, H, L, D] for internal computation
        queries = queries.transpose(2, 1)
        keys = keys.transpose(2, 1)
        values = values.transpose(2, 1)

        # ---- original ProbSparse formulas, but made safe for small L_K / L_Q ----
        # c * ln(L_k), c * ln(L_q)
        # use max(L, 2) so log(1) never gives 0, then clamp bounds
        U_part_raw = self.factor * np.ceil(np.log(max(L_K, 2))).astype("int").item()
        u_raw = self.factor * np.ceil(np.log(max(L_Q, 2))).astype("int").item()

        U_part = max(1, min(L_K, U_part_raw))
        u = max(1, min(L_Q, u_raw))

        scores_top, index = self._prob_QK(
            queries, keys, sample_k=U_part, n_top=u
        )  # scores_top: [B, H, u, L_K]
