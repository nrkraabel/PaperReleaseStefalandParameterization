...
#SBATCH --job-name=Camels531_CondensedDaily_PUB_LinearProbe
#SBATCH --array=0-2
...
# Ablation override: +model.nn.ablation_mode=linear_probe model.nn.adapter_type=none model.nn.use_residual_lstm=False
...
OUT_DIR=${DMG_OUTPUT_ROOT}/Extended1980_1999_Camels531/Camels_531_condensed_embeddings_daily_linearprobe/PUB/seed${SEED}/
...
python ${DMG_REPO}/src/dmg/__main__.py \
    ...
    +model.nn.ablation_mode=linear_probe model.nn.adapter_type=none model.nn.use_residual_lstm=False \
    save_path=$OUT_DIR \
    +out_path=$OUT_DIR \
    +run_dir=$OUT_DIR \
